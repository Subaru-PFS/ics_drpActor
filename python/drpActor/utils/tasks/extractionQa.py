...
class ExtractionQaTask(BaseTask):
    ...
    def runFromActor(self, file):
        ...
        self.parsedCmd.config = ExtractionQaTask.ConfigClass()
        ...
        try:
            userConfig = self.tasksExec.engine.config['extractionQa']
            self.mergeConfig(userConfig)

        except Exception as e:
            self.tasksExec.engine.logger.warning(f'failed to load user config for extractionQa with : {str(e)}')

        # send task to the ProcessPoolExecutor.
        future = self.tasksExec.executor.submit(runOutsideClass, self.parsedCmd)
        future.add_done_callback(partial(self.tasksExec.waitForExtractionQa, file))
